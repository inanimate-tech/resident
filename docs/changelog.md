...
- **BREAKING — one event payload shape: driver events deliver `event.data`.** Driver event fields are no longer flattened onto the event table (`event.index` → `event.data.index`): every event — driver and wire alike — now carries its payload as an `event.data` table, parsed by the same ArduinoJson rules. The hand-rolled flatten parser (which silently dropped booleans and nesting, and cut strings at the first quote) is deleted; `EventField` gains `FLOAT` and `BOOL` alongside `INT` and `STRING`. Migration for apps: `e.index` → `e.data.index` (all shipped example apps, DEVICE-SKILL.md files, and prompt sheets are updated). Firmware `sendEvent` call sites need no changes unless they want the new field types.
...
- **`prompts/` — canonical authoring sheets for coding agents.** `prompts/sandbox.md` (the universal Lua surface) and `prompts/lgfx.md` (the lgfx module), maintained next to the code they document and composed per board by consumers (agent skills, hosts, arc's profile authoring documents); see `prompts/README.md` for the composition rules. The create-app skill's embedded copy now carries a provenance banner and is synced from the canonical sheet — fixing its stale `kv` module section (the module has been `store` since it shipped).
