...
- `RenderTargets::addPanel` no longer asks the panel its size at registration — boards register in a config function that commonly runs during static init, where the display object does not exist yet (a LoadProhibited crash in global-ctor time on ESP32-S3 / core 3.3.9, with no serial output to explain it). Geometry is read at call time instead.
