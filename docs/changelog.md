...
- **luavgl is pinned to the fork's `main`.** The Inanimate luavgl fork now
  develops on `main`; its old integration branch is gone, so the
  `m5stick-lvgl` env's `#arc` pin no longer resolves. The env pins `#main`
  (pin a commit SHA for a frozen build). The fork's optional C base theme is
  renamed with it — `luavgl_arc_theme_init` → `luavgl_base_theme_init`, in
  `src/base_theme.c` — which no Resident code calls, because `LvglModule`
  installs no theme and apps set their own via `h:set_theme{...}`. Fork docs:
  its README section and `docs/display-bind.md`.

- **Bind is the claim: a library-agnostic render target with ownership (arc R19).** Boards now register ONE library-agnostic target per panel — `Resident::PanelTarget` (geometry + a synchronous RGB565 big-endian `blit(x, y, w, h, px)`) via `RenderTargets::addPanel(name, panel, shape)` — and the graphics modules build their own machinery over it when Lua binds: `LgfxModule` allocates and owns a full-frame sprite (new `LgfxSpriteTarget<T>` adapter: `createSprite` at the panel's geometry on first bind, present = one blit) and `LvglModule` owns the entire LVGL runtime (`lv_init`, the `millis` tick, the `lv_display_t` created on first bind, its draw buffers, the flush over the same blit, the `lv_timer_handler` pump, the app-reset tree wipe). **A board's display glue shrinks to "the panel, addressable"** — the m5stick-demo `LVGLDriver` is deleted outright and replaced by a 20-line `M5Panel`.
  **Ownership**: the registry holds an owner bit per target; `lgfx.bind(name)` / `lvgl.bind(name)` are both the app's library declaration and its ownership claim (last bind wins, either direction), and the NON-owner's present path stands down silently — an lgfx `flip()` is dropped, LVGL's flush returns without touching the panel and its refresh timer is paused (not `lv_display_set_default` juggling, which is global state other code reads). A takeover invalidates the whole active screen, because the glass belonged to the other library a moment ago. `onAppReset` releases every claim, so each app's first bind is a clean claim — which is also what kills the app-reset blank-frame race: an unowned display never flushes the blank frame its own wipe invalidated. The system/status display role and overlays are deliberately OUT of the arbitration (they write through the board's own path). New API: `PanelTarget`, `RenderTargets::addPanel/panel/claim/owner/isOwner/release/releaseAll`, `LgfxSpriteTarget<T>`, `LgfxTarget::allocate/pixels`, `LvglModule::DisplayOptions {dpi, bufferRows, buffer, bufferBytes}`. Native tests: `test_render_targets` (the registry and the ownership logic in full) plus four R19 cases in `test_lgfx_module` that drive claim/stand-down/release/reclaim through real Lua binds.
  **BREAKING** (within the same unreleased cycle that introduced them): `LvglModule::addDisplay(name, lv_display_t*, shape)` is replaced by `addDisplay(name[, DisplayOptions])` — the module creates the display now, so a board-created one cannot be gated and the shim was not worth its dishonesty; boards delete their LVGL glue driver and register a `PanelTarget` instead. `LgfxModule::addDisplay(name, target, shape)` is unchanged, and presenter-callback targets keep working (now subject to ownership).

- **The hello slims to session mechanics.** The `surfaces` array and the `features` list are removed: surface/sensor/module facts are AUTHORING material whose one authority is the document behind the `profile` key, and the feature toggles gated nothing (chunk support is part of protocol version 1; telemetry is handled as it arrives; capture announces itself via its bracket). The hello carries exactly what a host needs to operate a session: `protocol`, identity, `limits`, `framework`, `app`, and the `profile` key.
