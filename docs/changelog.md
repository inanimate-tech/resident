...
- **Wall-clock dispatch deadline**: `SandboxConfig::executionDeadlineMs` (default 0 = off) bounds a dispatch in time instead of instructions and takes precedence over `executionBudget` when set. It is lazily armed — no Lua hook is installed at dispatch start, and a one-shot `esp_timer` installs a stopping hook only once the deadline passes — so a dispatch that keeps to its deadline pays none of the trap cost an instruction cap imposes on every VM instruction. Measured on an ESP32-S3: a Lua-heavy shader runs 1.80x faster than under the instruction cap (830 ms → 462 ms per tick, versus 463 ms with no guard at all), and a bare `while true` aborts 1–9 ms past a 900 ms deadline, every dispatch, indefinitely. Device builds only.
- **Slow-dispatch reporting**: `SandboxConfig::executionSoftDeadlineMs` (default 0 = off) reports a dispatch that outlives it — a rate-limited serial line plus a `slow_dispatch` telemetry event — without aborting it. Measured at dispatch end, so unlike either hook it also sees time spent inside a blocking C binding. Available on host builds.
