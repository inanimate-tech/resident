...
describe("validateLuaCode", () => {
  it("accepts a valid m5stick app", async () => {
    const code = `
      function init(ctx)
        screen.clear()
    ...
    `
    const result = await validateLuaCode(code)
    expect(result.ok).toBe(true)
    expect(result.error).toBeUndefined()
  })

  it("rejects code that calls a nonexistent screen method", async () => {
  ...
  it("rejects code that fails to compile", async () => {
  ...
  it("runs on_tick five times against the stub set without crashing", async () => {
    const code = `
      local counter = 0
  ...
  it("accepts apps that only define init", async () => {
