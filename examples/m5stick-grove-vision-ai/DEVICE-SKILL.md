# M5StickS3 + Grove Vision AI V2

The M5StickS3 is a wrist-mountable ESP32-S3 device with a 135×240 colour TFT,
a 6-axis IMU, two physical buttons, and a piezo buzzer. It has a Grove port
connected to a **Grove Vision AI V2** camera module running a SenseCraft AI
model on-device. Apps drive the screen as their primary output, respond to
button presses + motion, and can react to live inference results from the camera.
...
**Grove Vision AI V2:** Camera module on the Grove port running a SenseCraft AI
model. Inference runs on the camera module itself; results are streamed to the
sandbox and exposed via the `vision.*` module and `"vision"` events.
...
| `"pose"`    | human pose (YOLOv8)                               | `x,y,w,h,score,target` (person box; keypoints via `vision.keypoint`) |
...
- scene became empty: `e.kind` (the kind that emptied), `e.n == 0` — sent
  once per transition
...
- Vision coordinates are in the model's frame (typically 192×192), not screen pixels.
...
- Vision coords are model-frame (typically 192×192) — scale to screen before drawing.
- Check `vision.ok()` before trusting results; `vision.age_ms()` tells you how stale the last frame is.
- Write apps that handle `vision.kind()` defensively — the model on the camera module can be reflashed.
...
and `vision` modules described above.
